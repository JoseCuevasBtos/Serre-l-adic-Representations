...
\todo[pinktask]{Cambiar $T$ por $\TT$ (pero no en $T_{\mathfrak{m}}$).}
...
\end{enumerate}
	...
	\begin{enumerate}[a)]
		...
		\todo[pinktask]{Cambiar $G_\mathfrak{m}$ por $\GG_m$.}
		\[
			\numnorm \colon S_{\mathfrak{m}} \rightarrow \GG_\mathfrak{m}
		\]
		such that $\numnorm(F_v) = \numnorm v \in \Q^{\times}$ for all 
		$v \not\in \Supp(\mathfrak{m})$.
		\item Show that the morphism 
		\begin{tikzcd}[cramped, sep=small]
		T \rar & S_\mathfrak{m} \rar["\numnorm"] & \GG_\mathfrak{m}
		\end{tikzcd}
...
$T(R) = (K \otimes R)^{\times} = \prod_{v \in M_K^\infty} K_v^{\times}$, 
...
\begin{enumerate}[1)]
	...
	defined in Exercise 2 of~\ref{sec:II_25}. Show that $\numnorm \cirv j 
	\colon \GG_m \to S_\mathfrak{m} \to \GG_m$ is $\lambda \mapsto \lambda^2$.
\todo{Referenciar ejercicio?}
	Show that any morphism $S_\mathfrak{m} \to \GG_m$ is equal to 
	$\varepsilon\numnorm^{n}$, where $\varepsilon$ is a character of 
	$C_\mathfrak{m}$ with values in $\{\pm 1\}$ and $n \in \Z$.
	...
	\begin{enumerate}[a)]
		\item Show that $dh$ is even (apply Exerc.~1 to $\det(\phi) \colon 
		S_\mathfrak{m} \to \GG_m$).
		\item Prove that there exists on $V_0$ a positive definite quadratic 
		form $\Q$ such that 
		\[
			\Q(\rho(x)y) = \numnorm(x)^d\Q(y)
		\]
		for any $y \in V_0$ and any $x \in S_\mathfrak{m}(\Q)$. [Let $H$ be the 
		kernel of $\numnorm \colon S_\mathfrak{m} \to \GG_m$. Using the fact 
		that $H(R)$ is compact, prove the existence of a positive definite 
		quadratic form $\Q$ on $V_0$ invariant by $H$; then note that 
		$S_\mathfrak{m}$ is generated by $HH$ and $j^{*}(\GG_m)$.] 
	\end{enumerate}
\end{enumerate}
