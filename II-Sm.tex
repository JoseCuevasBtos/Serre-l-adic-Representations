...
\label{sec:II_11}
Let $\TT = \WRes_{K/\Q}(\subbch{\GG}{m}{K})$\index{Tz@$\TT =
\WRes_{K/\Q}(\subbch{\GG}{m}{K})$} be the algebraic group over $\Q$, obtained
from the multiplicative group $\GG_m$\index{Gm@$\GG_m$} by restriction of
...
If $d = [K : \Q]$, the group $\TT$ is a \strong{torus}\index{Torus} of
...
group}\index{Character group} $X(\TT) = \Hom_{\algcl{\Q}}(\TT_{/\algcl{\Q}},
...
\overline{E \times E}$, one sees that $\overline{E}$ is an \emph{algebraic
subgroup} of $\TT$.
Let $\TT_E$ be the quotient group $\TT/\overline{E}$; then $\TT_E$ is also a
torus over $\Q$.
Its character group $X_E = X(\TT_E)$ is the subgroup of $X = X(T)$ consisting
of those characters which take the value 1 on $E$.
...
% \todo{actualizar notación de Galois}
...
k}(H_{/\algcl k}, \subbch{\GG}{m}{\algcl k})$ be the group of characters of $H$
...
\item Let $V$ be a finite-dimensional $k$-vector space and let
	...
	\strong{$\Rep_{k_1}(H_{/k_1})$ can be defined over $k$},\index{Defined
	over $k$ (representation)} if it is in the image of this map.
...
complex character $S_{\mathfrak{m}/\C} \to
\subbch{\GG}{\mathfrak{m}}{\C}$,
\todo[pinktask]{Originalmente el libro tiene
$\subbch{\GG}{m}{\C}$ puede que sea un error de tipeo del original, dejaremos la
corrección del profesor Schoof}
one gets a homomorphism $C \to \C^{\times}$, i.e.\ a \emph{Gr\"o\ss
encharakter} of K, in the sense of Hecke.
It is easily seen that the characters obtained in this way coincide with the
``Gr\"o\ss encharakter of type ($A_0$)'' of Weil (cf.\ \cite{35}, \cite{41}),
whose conductor divide $\mathfrak{m}$.
...
\item Show that the image of $e$ is contained in the subgroup
	$S_\mathfrak{m}(\A_\Q)$ of $S_\mathfrak{m}(\R) \times \prod_{\ell}
	S_\mathfrak{m}(\Q_\ell)$, where $\A_\Q$ denote the ring of adèles of $\Q$,
	and that $e \colon I \to S_\mathfrak{m}(\A_\Q)$ is continuous (for the
	natural topology of the adelized group $S_\mathfrak{m}(\A_\Q)$).
\item Let $\pi_\A \colon \TT(\A_\Q) \to S_\mathfrak{m}(\A_\Q)$ be the map defined
	by $\pi \colon \TT \to S_\mathfrak{m}$. Show that, if one
	identifies $\TT(\A_\Q)$ with $I$ in the obvious way, one has 
	\[
		e(x) = \varepsilon(x) \pi_\A(x^{-1})
	\]
...
\item Show that $\varepsilon(I)$ is not open in $S_\mathfrak{m}(\A_\Q)$ if
	$C_\mathfrak{m} \neq \{1\}$.
\end{enumerate}
...
have \textquote{complex multiplication}\index{Complex multiplication} by $E$;
in the terminology of Shimura-Taniyama, it is a variety of \textquote{type
(CM)}.
...
theorem \cite{39}).
Hence $\rho_\ell$ is \textquote{locally algebraic} (Chapter~\ref{ch:iii},
...
let $[\sigma] \in X(\TT)$ be the corresponding character of $\TT$; the
...
associated to $\phi$.
Then $\phi_\ell$ is integral (cf.\ Ch.~\ref{ch:i},~\ref{sec:I_22}) if
...
\end{prop}
\begin{proof}
If $\chi = a \sum_{\sigma \in \Gamma} [\sigma] + \sum_{\sigma \in \Gamma}
...
sum of those $Y_\lambda$ where for all the infinite Frobeniuses $c \in C_\infty$
