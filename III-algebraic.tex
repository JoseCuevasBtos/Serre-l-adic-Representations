...
\todo[pinktask]{$C$ son los complejos o qué} (cf.\ Tate \cite{39}, \S3.3). Hence,
...
to the inertia group of $G$ is $x \mapsto r_e(x^{-1})$, and hence \emph{is
...
\todo[pinktaks]{¿Es $C$ o $\C$?} where $d = [E : \Q_p]$. Every element $x$ of
...
isomorphic to $C(\sigma \icrc \rho)$.
\end{enumerate}
\end{lem}
...
\begin{enumerate}[(i)]
	\item $\rho \equiv \prod_{\sigma \in \Gamma_E} \sigma^{-1} \circ
\chi_{\sigma E}^{n_\sigma}$.
	\item $\sigma \circ \rho \sim \chi^{n_\sigma}$ for all $\sigma \in
\Gamma_E$
...
$\sigma E$ off $K$, as in \ref{sec:III_A4}. Note that, since $\chi_{\sigma E}$
...
$p \equiv \prod_{\sigma \in \Gamma_E} \sigma^{-1} \circ \chi_{\sigma
E}^{n_\sigma}$
\end{cor}

This follows from (iii) and the fact that $W = C \oplus V$ is the direct sum of
the $W_\sigma$'s.
