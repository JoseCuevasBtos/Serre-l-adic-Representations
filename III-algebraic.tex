...
Let $\rho \colon \abcl{\Gal(\overline{K}/K)} \to \Aut(V)$ be an abelian $p$-adic
representation of $K$ in $V$, where \abcl{$\Gal(\overline{K}/K)}$ denotes the
Galois group of the maximal abelian extension of $K$. If $i \colon K^\times \to
\abcl{\Gal(\overline{K}/K)} $ is the canonical homomorphism of local class field
...
continuous homomorphism $\abcl{\Gal(\overline{\Q}/\Q)} \to U_p$, where $U_p$ is the
...
\begin{prop}
	Let $\rho \colon \abcl{\Gal(\overline{K}/K)} \to \Aut(V)$ be a locally
algebraic abelian representation of $K$. The restriction of $\rho$ to the
inertia subgroup of $\abcl{\Gal(\overline{K}/K)}$ is semi-simple.
\end{prop}

\begin{proof}
Let us identify the inertia subgroup of $ \abcl{\Gal(\overline{K}/K)} $ with the
...
\frac{1}{(U_K:U')}\sum_{s \in U_K \setminus U'}
\rho(S) \pi \rho(s^{-1})$\todo[pinktask]{qué notación es (:)}
, we obtain a projector $\pi' \colon V \to W$ which
commutes with all $\rho(s)$, $s \in U_K$. 
\end{proof}
...
\[\begin{tikzcd}
	{K^\times} & {\abcl{\Gal(\overline{K}/K)}} \\
	{K'} & { \abcl{\Gal(\overline{K}/K') }
...
\abcl{\Gal(\algcl{K}/K)}$ be the corresponding decomposition group. This group
...
\[
	\rho \colon \Gal(\overline{K}/K) \to \Aut(V_\lambda)
\]
