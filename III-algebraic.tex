...
is a quotient of the local Galois group $\Gal(\algcl{K_v}/K_v)^{\ab}$ (there two
group are, in fact, isomorphic, but we do not need this here). Hence, we get by
composition an $\ell$-adic representation of $K_v$ 
\[
\rho_v \colon \Gal(\algcl{K_v}/K_v)^{\ab} \longrightarrow D_v
\overset{\rho}{\longrightarrow} \Aut(V_\ell).
\]
...
It is convenient to reformulate this definition, using the torus $T = R_{K/\Q}
(G_{m/K})$ of Chap.~\ref{ch:ii},~\ref{sec:II_11}. Let $T/{\Q_\ell} = T
\otimes_{\Q} \Q_\ell$ be the $\Q_\ell$-torus obtained from $\TT$ by extending the
ground field from $\Q$ to $\Q_\ell$. We have
\[
T/{\Q_\ell} (\Q_\ell) = (K \otimes \Q_\ell)^{\times} = K_{\ell}^\times,
\]
...
\colon I \to \Gal(\algcl{K}/K)^{\ab}$, define a homomorphism
\[
i_\ell \colon K_\ell^\times \longrightarrow \Gal(\algcl{K}/K)^{\ab}.
\]
...
algebraic morphism 
\[
f \colon T/{\Q_\ell} \longrightarrow \GL_{V_\ell}
\]

such that $\rho \circ i_\ell(x) = f(x^{-1})$ for all $x \in K_\ell^\times$ close
enough to $1$. 
\end{prop}
